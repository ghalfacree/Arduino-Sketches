// Convert image to a C header file suitable for the Pervasive Displays 2.7in ePaper module.
// Based on the Adafruit Thermal Printer image conversion sketch.
...
void setup() {
  ...
  output.println("#include \"Display_Controller.h\"");
  output.println("#if(DISPLAY_IN_USE == EPD_TYPE_270)");
  output.println();
  output.print("unsigned char const " + basename + "[176][33] = {");
  ...
  for(i=n=y=0; y<img.height; y++) { // Each row...
    ...
    for(x=0; x<rowBytes; x++) { // Each 8-pixel block within row...
      ...
      if((++n < totalBytes) & (x < (rowBytes -1))) output.print(",");
      if((x == (rowBytes - 1)) & (y < (img.height -1))) output.print("},");
      if((x == (rowBytes - 1)) & (y == (img.height -1))) output.print("}");
    }
  }
  ...
  output.println();
  output.println("};");
  output.println("unsigned char PreloadImage[176][33];");
